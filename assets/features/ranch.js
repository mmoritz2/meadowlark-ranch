...
export function install(G){
 ...
 /* Daily rows. story-quests normalises every row it can see to sp:10 when it installs, and
    it installs before this package — so these carry their Star Points themselves. A row
    whose type another package already claimed (story-quests owns 'build') is left alone:
    two rows sharing a type would share one progress counter and one claim. */
 for(const row of [{type:'stallin',icon:'🏠',label:'Bring a horse into the barn',goal:1,r:{c:60,g:2,p:10,sp:10}},
                   {type:'build',icon:'🏗️',label:'Place 3 pieces of decor',goal:3,r:{c:90,g:2,p:10,sp:10}}])
  if(!G.quest.DAILYQ.some(q=>q.type===row.type))G.quest.addDaily(row);
 ...
 G.on('decorPlaced',(d,c)=>{ refreshCaches(); if(c.cat==='indoor')G.quest.dailyEvt('furnish',1); if(c.set&&DECOR_SETS[c.set]&&setProgress(S.fresh()||{},c.set)===DECOR_SETS[c.set].pieces.length){G.sGem();toast('🧩 '+DECOR_SETS[c.set].label+' complete — '+DECOR_SETS[c.set].text+'!');} });
