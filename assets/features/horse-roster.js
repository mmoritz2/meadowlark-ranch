...
export function install(G){
 ...
 function perkFor(breed,M){const P=BREED_PERKS[breed];if(!P)return null;const out={sp:0,ac:0,ag:0,jp:0,stam:1,labels:[]};for(const r of [5,10]){if(M>=r&&P[r]){const q=P[r];out.sp+=q.sp||0;out.ac+=q.ac||0;out.ag+=q.ag||0;out.jp+=q.jp||0;out.stam*=q.stam||1;out.labels.push(q.label);}}return out;}
 function perkLine(breed,M){const P=BREED_PERKS[breed];if(!P)return '';return [5,10].map(r=>(M>=r?'✅ ':'🔒 ')+r+' · <b>'+esc(P[r].label)+'</b> '+perkDesc(P[r])).join(' &nbsp; ');}
 ...
 function refreshCur(){
  ...
  const s=G.save.fresh(); cur.M=s?G.xp.masteryOf(s,h.breed):0; cur.traits=traitsOf(h); cur.flags=flagsOf(h); cur.abil=h.ability||cur.flags.ability||null; cur.perk=perkFor(h.breed,cur.M);
 }
 ...
 G.on('ride',(R,dt)=>{
  ...
  if(p.flying){
   const M=cur.M;
   if(!cur.wasFlying){cur.wasFlying=true;if(M<1)p.stam=Math.max(0,(p.stam==null?1:p.stam)-0.15);}
   if(R.gallop&&R.fwd){p.stam=Math.max(0,(p.stam==null?1:p.stam)-dt*0.02*(M>=10?0.7:1)*drain);R.regen=0;}   // a fast flight spends stamina; a glide gets it back
   if(M>=4&&R.gallop&&(p.climbInput||0)<0)p.flyAlt=Math.max(0,(p.flyAlt||0)-4*dt);   // Wing Tuck: a dive, not level flight
   if(M>=10&&R.fwd)R.target*=1.15;
   if(M>=10&&p.boostT>0)p.boostT+=dt*0.3;
 ...
 G.on('tick',(dt,t)=>{
  ...
  const tr=p.trick; if(tr&&tr!==cur.lastTrick){cur.lastTrick=tr;if(p.flying&&cur.M>=7){G.xp.passAdd(2);cur.trickN++;if(cur.trickN%5===1)toast('🪽 Aerial Acrobatics! +2 pass points');}}
 ...
 G.ui.careSection((s,h)=>{const M=G.xp.masteryOf(s,h.breed);let x='';
  ...
  if(h.wings)x+='<div style="font-size:11.5px;margin-top:4px">🪽 <b>Flight mastery</b> · '+Object.keys(FLIGHT_UNLOCKS).map(r=>(M>=+r?'✅ ':'🔒 ')+r+' '+esc(FLIGHT_UNLOCKS[r])).join(' &nbsp; ')+'</div>';
 ...
 if(!T.EVENTS3.some(e=>e.id==='l1'))T.EVENTS3.push({id:'l1',town:'Loon Lake',name:'Loon Lake Crossing',lvl:3,race:true,route:'ll',reward:450});
 ...
 G.quest.addAch({id:'peg10',icon:'🪽',label:'Soaring speed',desc:'Reach flight mastery 10',v:s=>Math.max(0,...s.horses.filter(h=>h.wings).map(h=>G.xp.masteryOf(s,h.breed)),0),goal:10,r:{g:5}});

 /* ---- 10. exports for the other packages ----------------------------------------------- */
 G.horse.roster={TIER_STARS,COATS3,TRAITS3,VARIANT_RECIPES,DRAGON_FAMILY,BREED_PERKS,FLIGHT_UNLOCKS,FANTASY_CEIL,NEW_THEMES:Object.keys(NEW_THEMES),SEASON_CALL_GEMS,SEASON_CALL_PITY,
