/* +X-forward / +Y-up equine motion. Stance travels at -root speed; analytic
 * limb IK and hoof counter-rotation solve each pose without editing the mesh. */
export const HERO_GAITS={
...
const ORDER=[{id:'LF',ids:[13,14,15,16,17,18],front:true},{id:'RF',ids:[7,8,9,10,11,12],front:true},{id:'LH',ids:[28,29,30,31,32],front:false},{id:'RH',ids:[23,24,25,26,27],front:false}];
...
export function createHeroMotion({THREE,root,skin,heightM=2.3}){
  if(!skin?.isSkinnedMesh||skin.skeleton.bones.length!==33)return null;
  ...
  root.updateMatrixWorld(true);worldToRoot.copy(root.matrixWorld).invert();const rootQ=root.getWorldQuaternion(Q()).invert();
  ...
  const metres=heightM/(topY-groundY);
  const legs=ORDER.map(spec=>{
    const ids=spec.ids,terminal=ids.at(-1),fetlock=ids.at(-2),candidate=[];let low=Infinity;
    ...
    return {...spec,terminal,fetlock,restSole,soleVertex,samples,soleOffset:restSole.clone().sub(restP[terminal]),pastern:restP[terminal].clone().sub(restP[fetlock]),target:V(),oldTarget:V(),nextTarget:V(),foot:V(),ankle:V(),hinge:V(),middle:V(),contact:true,pitch:0,reachError:0};
  });
  ...
  const params=state=>HERO_GAITS[state.gait]||HERO_GAITS.stand;
  ...
  function sampleBody(state,out){
    ...
    if(state.gait==='jump'){
      ...
      out.pitch=air?.13*Math.cos(Math.PI*u):a<.38?.13*ease(a/.38):-.13*(1-ease((a-1.16)/.36));out.grounded=!air;return;
    }
    if(state.gait==='stand'){out.y=.0015*Math.sin(time*TAU/4);out.pitch=.0012*Math.sin(time*TAU/8);return;}
  ...
  function targetFor(state,leg,index,out){
    ...
    if(state.gait==='jump'){
      ...
      out.y+=((a>=.38&&a<=1.16?4*.55*flight*(1-flight):0)+(fore?.31:.23)*tuck)*ease((end-a)/.13);out.x+=(fore?.16:-.07)*tuck;if(!air)out.y=groundY;
    ...
    out.x=restP[leg.ids[0]].x+(leg.front?.025:0);
    ...
    else{const u=(phase-duty)/(1-duty),s=ease(u),derivative=-stride*(1-duty),arc=Math.sin(Math.PI*u)**(state.gait==='walk'?2.2:1.7),tangent=u*(1+16*u)*(1-u)**16-(1-u)*(1+16*(1-u))*u**16;out.x+=-span*.5+span*s+derivative*tangent;out.y+=(leg.front?1:.84)*g.lift*arc;out.z+=(index===0||index===2?1:-1)*.012*arc;pitch=-(leg.front?.56:.38)*arc;}
  ...
  function fk(){
    ...
    for(let i=0;i<n;i++){
      ...
      let pitch=0;if(i===0)pitch=bodyPitch;if(i===2)pitch=-bodyPitch*.25;if(i===4)pitch=(.006+.016*intensity)*Math.sin(ph+.55)+air*.035;if(i===5)pitch=-.007*Math.sin(ph+.9)+.008*Math.sin(secondary);if(i===6)pitch=.003*Math.sin(secondary*2);
      if(pitch)q[i].premultiply(delta.setFromAxisAngle(axisZ,pitch));if(i===5)q[i].premultiply(delta.setFromAxisAngle(axisY,.009*Math.sin(secondary)));
      if(i>=19&&i<=22){const lag=(i-19)*.44; q[i].premultiply(delta.setFromAxisAngle(axisX,.025*Math.sin(secondary*2-lag)+.013*intensity*Math.sin(ph-lag)+turn*.022));}
    }
  }
  ...
  function apply(){
    for(let i=0;i<n;i++){const parent=parents[i];if(parent<0){bones[i].position.copy(p[i]);bones[i].quaternion.copy(q[i]);}else{inverse.copy(q[parent]).invert();bones[i].position.copy(p[i]).sub(p[parent]).applyQuaternion(inverse);bones[i].quaternion.copy(inverse).multiply(q[i]).normalize();}}
  ...
  function step(dt){
    ...
    for(const leg of legs){footTargets(leg);const hip=p[leg.ids[0]],reach=upperReach(leg)+restP[leg.ids[2]].distanceTo(restP[leg.fetlock])-.014,horizontal=(hip.x-leg.ankle.x)**2+(hip.z-leg.ankle.z)**2;const allowed=Math.sqrt(Math.max(.001,reach*reach-horizontal));supportDrop=Math.max(supportDrop,hip.y-leg.ankle.y-allowed);}
    if(supportDrop>0){bodyY-=supportDrop;fk();}
  ...
  function set(gait,{lead=current.lead,speed}={}){
    if(!HERO_GAITS[gait])gait='stand';lead=lead==='right'?'right':'left';const requestedSpeed=gait==='jump'?0:Math.max(0,Math.min((HERO_GAITS[gait].speed||0)*1.15,speed??HERO_GAITS[gait].speed));
  ...
  function snapshot(){return {gait:current.gait,requestedGait:current.gait,lead:current.lead,phase01,cycleHz:params(current).hz,speedMps,distanceM:distanceRaw*metres,transitioning:!!previous,grounded,time,age:current.age,metresPerUnit:metres,
    feet:legs.map(leg=>{const sole=actualPoint(leg.soleVertex,V()),target=leg.target.clone();target.y-=groundY;target.multiplyScalar(metres);let minimum=Infinity;for(const index of leg.samples)minimum=Math.min(minimum,actualPoint(index,temp3).y);return{id:leg.id,contact:leg.contact,sole:sole.toArray(),worldSole:[sole.x+distanceRaw*metres,sole.y,sole.z],targetSole:target.toArray(),soleMinY:minimum,reachError:leg.reachError*metres};}),
    bonePositions:p.map(v=>[v.x*metres,(v.y-groundY)*metres,v.z*metres]),boneQuaternions:q.map(v=>v.toArray()),localPositions:bones.map(b=>b.position.toArray()),localQuaternions:bones.map(b=>b.quaternion.toArray())};}
  reset();return {set,reset,snapshot,get mode(){return current.gait;},get state(){return {speedMps,phase01,lead:current.lead,intensity:clamp(speedMps/7.35),grounded,turn,distanceM:distanceRaw*metres,bodyLiftM:bodyY*metres};},setTurn(value){turn=clamp(value,-1,1);},update(dt){let remaining=clamp(dt,0,.25);while(remaining>1e-8){const h=Math.min(remaining,1/120);step(h);remaining-=h;}}};
}
