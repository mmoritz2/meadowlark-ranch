...
(async()=>{
 ...
 const A=await page.evaluate(()=>{
  ...
  out.ceil={aetherSpeed:G.xp.statCeil({breed:'aether'},'speed'),aetherJump:G.xp.statCeil({breed:'aether'},'jump'),glacierAccel:G.xp.statCeil({breed:'glacier'},'accel'),alicornAll:G.xp.statCeil({breed:'alicorn'},'agility'),bayNoK:G.xp.statCeil({breed:'bay'})};
 ...
 check('per-fantasy stat ceilings (aether speed 10 / jump 8, glacier accel 7, alicorn 10)',A.ceil.aetherSpeed===10&&A.ceil.aetherJump===8&&A.ceil.glacierAccel===7&&A.ceil.alicornAll===10&&A.ceil.bayNoK===8,A.ceil);
 ...
 const C=await page.evaluate(()=>{
  ...
  out.panel={shown:p.style.display,btnAfter:document.getElementById('catalogBtn').previousElementSibling.id,title:/Horse Catalogue/.test(p.textContent),alicornBred:/Alicorn/.test(p.textContent)&&/Bred from/.test(p.textContent),stars:(p.textContent.match(/⭐/g)||[]).length};
 ...
 check('catalogue panel: dock button after Stable, title, six-star sources, stars',C.panel.shown==='flex'&&C.panel.btnAfter==='stableBtn'&&C.panel.title&&C.panel.alicornBred&&C.panel.stars>100,C.panel);
 ...
 const fly=await page.evaluate(()=>{const G=window.__features,p=G.horse.player;p.pos.set(-40,0,-10);p.speed=0;p.stam=1;p.blown=false;window.advanceTime(300);
  ...
  G.save.sync(sv=>{sv.mastery.pegasus=10;}); G.horse.roster.refreshCur(); out.M=G.horse.roster.cur.M; p.stam=1;p.blown=false;
 ...
 check('flight economy: a fast flight drains stamina; Soaring Speed (mastery 10) drains 30% less and cruises faster',fly.stillFlying0&&fly.stillFlying10&&fly.stam0<0.95&&(1-fly.stam10)<(1-fly.stam0)*0.75&&fly.speed10>fly.speed0*1.05&&fly.M===10,{stam0:fly.stam0,stam10:fly.stam10,speed0:fly.speed0,speed10:fly.speed10,f0:fly.stillFlying0,f10:fly.stillFlying10});
