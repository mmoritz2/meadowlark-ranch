...
(async()=>{
 ...
 const r3=await run(page,`
  ...
  const ev=G.tables.EVENTS3.find(e=>e.id==='${seed3.jumps[0]}'); const RB=G.course.awardRibbons(ev,1,0); G.run('courseFinish',{c:null,ev,stars:3,RB,pay:0,dressage:false});
 ...
 check('claim moves on to Making a Name',r3.claim&&r3.next==='Earn 15 ribbons of any kind',r3.next);
 ...
 const r4=await run(page,`
  ...
  G.quest.questEvt('gallop',500); out.claim5=claim(); goto(20,16); window.advanceTime(200); out.visit=sv().story.prog; out.claim6=claim(); G.quest.questEvt('event','h1'); out.claim7=claim();
 ...
 check('gallop + visit + event missions still work after the inserts',r4.claim5&&r4.visit===1&&r4.claim6&&r4.claim7,{c5:r4.claim5,visit:r4.visit,c6:r4.claim6,c7:r4.claim7});
 ...
 check('case closed: counted, clues cleared, next mission',r4.afterClues&&r4.afterClues.cases===1&&r4.afterClues.clues.length===0&&r4.afterClues.label==='Gallop 600 m along the river',r4.afterClues);
