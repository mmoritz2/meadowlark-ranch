...
(async()=>{
 ...
 const A=await page.evaluate(()=>{
  ...
  let sx=null; G.save.sync(sv=>{const hh=sv.horses[0];hh.level=Math.max(hh.level||1,10);hh.stats.accel=3;hh.sxp.accel=0;G.xp.grantStatXp(sv,hh,'accel',100);sx={v:hh.stats.accel,left:hh.sxp.accel};}); out.sxpGain=sx;   // Lv10 lifts the level cap to 6; 156 XP (1.25 perk × 1.25 breed trait): 50 to reach 4, 60 to reach 5, 46 left
 ...
 check("Quick Learner: 'sxp' factor 1.25 × the Quarter Horse accel trait 1.25 on accel, 1 on speed, and 100 stat XP lands as 156 (3→5, 46 left)",Math.abs(A.sxpAccel-1.5625)<0.001&&A.sxpSpeed===1&&A.sxpGain&&A.sxpGain.v===5&&A.sxpGain.left===46,{a:A.sxpAccel,s:A.sxpSpeed,gain:A.sxpGain});
